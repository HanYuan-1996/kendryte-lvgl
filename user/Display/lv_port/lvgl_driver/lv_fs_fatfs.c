...
static void fs_init(void) {
   ...
   * Better to do it in your code to keep this library utouched for easy
   * updating*/
  ...
  LV_ASSERT_MSG(sd_init() == 0, "SD card err");

  FRESULT status = f_mount(&sdcard_fs, _T("0:"), 1);
  LV_ASSERT_MSG(status == FR_OK, "FAT32 err");
}
