...
void DialplateView::SetSpeedMeter(int speed) {
  ...
  /* anim */
  for (int i = 0; i < 4; i++) {
    lv_obj_add_anim((lv_obj_t*)&info[i], NULL,
                    [](void* obj, int var) {
                      indicInfo_t* info = (indicInfo_t*)obj;
                      if (info->meter == NULL || info->indic == NULL) return;
                      lv_meter_set_indicator_end_value(info->meter, info->indic,
                                                       var);
                    },
                    info[i].indic->end_value, Speed[i], 1000),
        NULL, lv_anim_path_linear;
  }
}
...
void DialplateView::TopInfo_Create(lv_obj_t* par) {
  ...
  lv_meter_set_scale_ticks(meter, scale, 51, 2, 5,
  ...
  lv_meter_indicator_t* indic1 = lv_meter_add_arc(
      meter, scale, 5, lv_palette_main(LV_PALETTE_LIGHT_GREEN), 0);
  lv_meter_indicator_t* indic2 =
      lv_meter_add_arc(meter, scale, 5, lv_color_hex(0xff931e), 5);
  lv_meter_indicator_t* indic3 =
      lv_meter_add_arc(meter, scale, 5, lv_palette_main(LV_PALETTE_RED), 10);
  lv_meter_indicator_t* needle =
      lv_meter_add_needle_line(meter, scale, 2, lv_color_white(), -7);

  ui.topInfo.meterSpeed = {meter, indic1, indic2, indic3, needle};
}
...
void DialplateView::SubInfoGrp_Create(lv_obj_t* par, SubInfo_t* info,
                                      const char* unitText) {
  lv_obj_t* cont = lv_obj_create(par);
  lv_obj_remove_style_all(cont);
...
lv_obj_t* DialplateView::Btn_Create(lv_obj_t* par, const void* img_src,
                                    lv_coord_t x_ofs) {
  ...
  static const lv_style_prop_t prop[] = {LV_STYLE_WIDTH, LV_STYLE_HEIGHT,
                                         LV_STYLE_PROP_INV};
...
#define ANIM_DEF(start_time, obj, attr, start, end)       \
  {                                                       \
    start_time, obj, LV_ANIM_EXEC(attr), start, end, 500, \
...
#define ANIM_OPA_DEF(start_time, obj) \
  ...
  lv_anim_timeline_t anim_timeline[] = {
      ANIM_DEF(0, ui.topInfo.cont, y, -lv_obj_get_height(ui.topInfo.cont),
               y_tar_top),

      ANIM_DEF(200, ui.bottomInfo.cont, y,
               -lv_obj_get_height(ui.bottomInfo.cont), y_tar_bottom),
