...
static void lcd_polling_enable(void) { lcd_ctl.mode = 0; }

static void lcd_interrupt_enable(void) { lcd_ctl.mode = 1; }
...
static int mcu_lcd_init(lcd_para_t *lcd_para) {
  ...
  lcd_ctl.width = lcd_para->width;
  lcd_ctl.height = lcd_para->height;
  ...
  // lcd_para->width, lcd_para->height, lcd_para->freq, lcd_para->invert,
  // lcd_para->offset_w0, lcd_para->offset_w1, lcd_para->offset_h1,
  // lcd_para->offset_h0, lcd_para->oct);
  if (g_lcd_w != lcd_para->width || g_lcd_h != lcd_para->height) {
    if (g_lcd_display_buff) {
      free(g_lcd_display_buff);
    }
    g_lcd_display_buff =
        (uint16_t *)malloc(lcd_para->width * lcd_para->height * 2);
    if (!g_lcd_display_buff) return 12;  // ENOMEM
...
static uint16_t mcu_lcd_get_width() { return g_lcd_w; }

static uint16_t mcu_lcd_get_height() { return g_lcd_h; }
...
static void mcu_lcd_set_direction(lcd_dir_t dir) {
  ...
  lcd_ctl.dir =
      ((lcd_ctl.dir & DIR_RGB2BRG) == DIR_RGB2BRG) ? (dir | DIR_RGB2BRG) : dir;
...
uint32_t mcu_lcd_get_freq() { return lcd_freq; }
...
static void mcu_lcd_draw_char(uint16_t x, uint16_t y, char c, uint16_t color) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    for (j = 0; j < 8; j++) {
      if (data & 0x80) mcu_lcd_draw_point(x + j, y, color);
      data <<= 1;
    }
    y++;
  }
...
static void mcu_lcd_draw_string(uint16_t x, uint16_t y, char *str,
                                uint16_t color) {
...
static void lcd_ram_draw_string(char *str, uint32_t *ptr, uint16_t font_color,
                                uint16_t bg_color) {
  // uint8_t i = 0;
  // uint8_t j = 0;
  // uint8_t data = 0;
...
static void mcu_lcd_fill_rectangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
static void lcd_draw_rectangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
static void mcu_lcd_draw_picture(uint16_t x1, uint16_t y1, uint16_t width,
  ...
  // 1, width, height, g_pixs_draw_pic_size);
  /*
...
#endif
    lcd_set_area(x1 + width - 1, y1 + height - 1, x1 + width - 1,
                 y1 + height - 1);
...
static void mcu_lcd_draw_pic_roi(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                                 uint16_t rx, uint16_t ry, uint16_t rw,
                                 uint16_t rh, uint8_t *ptr) {
  int y_oft;
  uint8_t *p;
...
static void mcu_lcd_draw_pic_gray(uint16_t x1, uint16_t y1, uint16_t width,
                                  uint16_t height, uint8_t *ptr) {
  uint32_t i;
  lcd_set_area(x1, y1, x1 + width - 1, y1 + height - 1);
...
static void mcu_lcd_draw_pic_grayroi(uint16_t x, uint16_t y, uint16_t w,
                                     uint16_t h, uint16_t rx, uint16_t ry,
                                     uint16_t rw, uint16_t rh, uint8_t *ptr) {
  int y_oft;
  uint8_t *p;
  for (y_oft = 0; y_oft < rh; y_oft++) {  // draw line by line
...
static void lcd_ram_cpyimg(char *lcd, int lcdw, char *img, int imgw, int imgh,
                           int x, int y) {
  int i;
  for (i = 0; i < imgh; i++) {
